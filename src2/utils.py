...
class ClassificationLearner(object):
    ...
    def predict(self, dls_test, compute):
        ...
        print("{:<10s} : {:.4f} ± {:.2f}".format("Loss", np.mean(loss), np.std(loss)))
        ...
        for l in compute:
            ...
            print("{:<10s} : {:.4f}".format(l, score))

        return {"labels" : labels, "preds" : preds, "probs" : probs}
                
    def train(self, n_epochs):    
        for epoch in tqdm(range(n_epochs)):
            train_loss, train_accuracy = self._fit()
            valid_loss, valid_accuracy = self._evaluate()

            if self.log_config is None:
                print(f"Epoch [{epoch}]: train_loss {train_loss} valid_loss {valid_loss} train_accuracy {train_accuracy} valid_accuracy {valid_accuracy}")
